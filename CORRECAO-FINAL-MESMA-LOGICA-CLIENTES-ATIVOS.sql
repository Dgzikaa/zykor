-- ============================================================
-- CORREÇÃO FINAL: Usar mesma lógica de clientes-ativos
-- ============================================================
-- Base Ativa = Clientes com 2+ visitas nos últimos 90 dias do fim do trimestre
...
-- ✅ MESMA LÓGICA DE /relatorios/clientes-ativos
-- Janela de 90 dias que termina no último dia do trimestre
base_ativa AS (
  SELECT 
    k.bar_id,
    k.ano,
    ...
    (
      ...
      FROM (
        SELECT cli_fone
        FROM public.contahub_periodo cp
        WHERE cp.bar_id = k.bar_id
          -- Janela de 90 dias antes do fim do trimestre
          AND cp.dt_gerencial >= (
            LEAST(
              CASE k.trimestre
          ...
          -- Até o fim do trimestre (ou hoje se ainda não terminou)
          AND cp.dt_gerencial <= LEAST(
            CASE k.trimestre
              WHEN 1 THEN (k.ano || '-03-31')::DATE
              WHEN 2 THEN (k.ano || '-06-30')::DATE
              WHEN 3 THEN (k.ano || '-09-30')::DATE
        ...
        GROUP BY cli_fone
        HAVING COUNT(DISTINCT cp.dt_gerencial) >= 2  -- 2+ DIAS com visitas
      ) subq
    )::numeric as base_ativa_90d
  FROM keys k
),
fat_contahub AS (
  SELECT bar_id,
...
-- Atualizar função RPC (já estava correta)
...
-- Verificar resultado
SELECT jobid, jobname, schedule FROM cron.job WHERE jobname LIKE 'refresh_view_visao_geral%';
