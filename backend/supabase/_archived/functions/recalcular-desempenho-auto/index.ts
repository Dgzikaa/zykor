...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const hbResult = await heartbeatStart(supabase, 'recalcular-desempenho-auto', null, 'recalculo', 'pgcron');
    ...
    // 💓 Heartbeat: registrar sucesso (com alerta Discord se partial)
    const finalStatus = erros === 0 ? 'success' : 'partial';
    await heartbeatEnd(
      supabase,
      heartbeatId,
      finalStatus,
      startTime,
      sucessos,
      { sucessos, erros, total: semanas?.length || 0 },
      undefined, // errorMessage
      'recalcular-desempenho-auto', // jobName para alerta Discord
      undefined  // barId
    );
