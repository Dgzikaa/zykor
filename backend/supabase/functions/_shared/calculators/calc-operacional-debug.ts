...
export async function calcOperacional(
  ...
  try {
    ...
    // 4. Atrasinhos do eventos_base
    const { data: eventosTempoData, error: eventosTempoError } = await supabase
      .from('eventos_base')
    ...
    if (eventosTempoError) {
      return {
        success: false,
        error: `Erro ao buscar eventos tempo: ${eventosTempoError.message}`,
        duration_ms: Date.now() - startTime,
      };
    }

    const atrasinhosBar = (eventosTempoData || []).reduce((sum: number, e: any) => sum + (Number(e.atrasinho_bar) || 0), 0);
    const atrasinhosCozinha = (eventosTempoData || []).reduce((sum: number, e: any) => sum + (Number(e.atrasinho_cozinha) || 0), 0);
    const atrasoBar = (eventosTempoData || []).reduce((sum: number, e: any) => sum + (Number(e.atrasao_bar) || 0), 0);
    const atrasoCozinha = (eventosTempoData || []).reduce((sum: number, e: any) => sum + (Number(e.atrasao_cozinha) || 0), 0);
