...
const sql = `DROP MATERIALIZED VIEW IF EXISTS public.view_visao_geral_trimestral CASCADE;
...
async function executeSql() {
  try {
    ...
    // Tentar usar PostgREST para executar SQL direto
    ...
    if (!response.ok) {
      ...
      // Se não houver função exec_sql, vamos informar ao usuário
