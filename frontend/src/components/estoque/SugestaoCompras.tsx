    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <p className="text-xs text-muted-foreground">
          Estoque atual + consumo médio/dia × cobertura − estoque. Exclui preparos (Produção).
        </p>
      ...
      {contagemBase && (
        <div className="text-xs text-amber-700 dark:text-amber-400 flex items-center gap-1.5">
          <AlertTriangle className="w-3.5 h-3.5" /> Baseado na última contagem disponível ({new Date(contagemBase + 'T00:00:00').toLocaleDateString('pt-BR')}). Faça uma contagem recente para refletir o estoque de hoje.
        </div>
      )}
