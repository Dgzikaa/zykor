                                      ...
                                      <div className="grid md:grid-cols-2 gap-3">
                                        ...
                                        <div>
                                          ...
                                          {dia.divergencias.so_ch.length === 0 ? <div className="text-[11px] text-muted-foreground">—</div> : (
                                            <table className="text-[11px] w-full"><thead className="text-muted-foreground"><tr><th className="text-left py-0.5 pr-2">Tipo</th><th className="text-left py-0.5 pr-2">Cliente</th><th className="text-left py-0.5 pr-2">Mesa</th><th className="text-left py-0.5 pr-2">Meio</th><th className="text-left py-0.5 pr-2">Garçom</th><th className="text-left py-0.5 pr-2">Comanda</th><th className="text-right py-0.5">Valor</th></tr></thead>
                                              <tbody>{dia.divergencias.so_ch.map((t: any, i: number) => { const est = Number(t.valor) < 0; return (<tr key={i} className={`border-t border-border/40 ${est ? 'bg-amber-100/70 dark:bg-amber-900/20' : ''}`}><td className="py-0.5 pr-2">{est && <span className="text-amber-700 dark:text-amber-300 font-semibold mr-1">estorno</span>}{t.tipo}</td><td className="py-0.5 pr-2">{t.cliente || '—'}</td><td className="py-0.5 pr-2 text-muted-foreground">{t.mesa || '—'}</td><td className="py-0.5 pr-2 text-muted-foreground">{t.meio || '—'}</td><td className="py-0.5 pr-2 text-muted-foreground">{t.garcom || '—'}</td><td className="py-0.5 pr-2 text-muted-foreground font-mono">{t.comanda || '—'}</td><td className="py-0.5 text-right font-medium">{est ? <span className="text-amber-700 dark:text-amber-300 font-bold inline-flex items-center gap-1" title="Estorno/ajuste — investigar se era pra estornar mesmo"><Undo2 className="w-3.5 h-3.5" />{fmtBRL(t.valor)}</span> : fmtBRL(t.valor)}</td></tr>); })}</tbody>
                                            </table>
                                          )}
                                        </div>
                                      </div>
