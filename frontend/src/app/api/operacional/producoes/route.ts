...
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await supabase
    .from('producao_base')
    .select('id,codigo,nome,unidade,rendimento,unidade_contagem,fator_contagem,secao,ativo,observacao,atualizado_em,controle_producao,curva_a,entra_contagem')
    .eq('bar_id', barId)
...
export async function PUT(request: NextRequest) {
  ...
  for (const k of ['nome', 'codigo', 'unidade', 'unidade_contagem', 'secao', 'observacao', 'ativo', 'controle_producao', 'curva_a', 'entra_contagem']) if (k in body) patch[k] = body[k];
