...
// body: { ano, mes, artPorDow: {0..6:%}, prodPorDow: {0..6:%} }  (dow: 0=dom..6=sáb; % em pontos, ex.: 15)
export async function POST(request: NextRequest) {
  ...
  for (const ev of eventos || []) {
    ...
    const artPct = Number(artPorDow[dow]);
    const prodPct = Number(prodPorDow[dow]);
    if (artPct > 0) patch.c_artistico_plan = Math.round(base * (artPct / 100) * 100) / 100;
    if (prodPct > 0) patch.c_prod_plan = Math.round(base * (prodPct / 100) * 100) / 100;
