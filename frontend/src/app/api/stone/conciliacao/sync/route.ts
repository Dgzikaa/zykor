...
export async function POST(req: NextRequest) {
  ...
  // Credenciais Stone do bar — UMA por CNPJ. Cada bar fatura em 2 CNPJs (ex.:
  // Ordinário + Ordibar), com chaves diferentes; cada chave só "enxerga" os
  // StoneCodes do seu CNPJ. Lê todas e usa a chave de cada uma com os codes dela.
    ...
    return NextResponse.json(
      {
        error:
          ...
          'Rode scripts/cadastrar-credencial-stone.mjs com a chave de Conciliação.',
      },
      { status: 404 },
    );
  }

  // Filtro opcional: testar um StoneCode específico (acha sozinho a chave do CNPJ dele).
  const filtroCode = body?.stone_code ? String(body.stone_code).trim() : null;
  ...
  for (const cred of creds) {
    ...
    try {
      ...
      resultados.push({ empresa: cred?.empresa_nome ?? null, ok: false, erro: e?.message || 'Falha ao resolver credencial' });
      continue;
    }

    const authHeader = stoneBasicAuthHeader(resolved.apiKey);
    const codes = filtroCode ? resolved.stoneCodes.filter((c) => c === filtroCode) : resolved.stoneCodes;
    ...
    for (const code of codes) {
      ...
      try {
        ...
        resultados.push({ empresa: resolved.empresaNome, stone_code: code, ok: false, erro: e?.message || 'Falha no fetch' });
      }
    }
  }
