...
export default function CmvTeoricoPage() {
  ...
  // Cardápio expansível: guarda os códigos internos abertos (mostra as sub-linhas por origem CH/Yuzer).
  const [expandido, setExpandido] = useState<Set<string>>(new Set());
  const toggleExp = (cod: string) => setExpandido(s => { const n = new Set(s); n.has(cod) ? n.delete(cod) : n.add(cod); return n; });
    ...
    <PageShell width="wide">
        ...
        {modo === 'cardapio' ? (<>
          ...
          <div className="flex items-center justify-between gap-2">
            <p className="text-xs text-gray-500 dark:text-gray-400">{view.length} produtos · clique na linha pra ver os códigos <b>ContaHub</b> e <b>Yuzer</b> (cada um com preço/CMV próprio)</p>
            {codigosComOrigem.length > 0 && <button onClick={toggleTodos} className="text-xs text-amber-600 hover:underline">{todosAbertos ? 'Recolher todos' : 'Expandir todos'}</button>}
          </div>
                  ...
                  <Fragment key={p.produto_id}>
                    ...
                    {expandivel && aberto && origens.map((o: any, i: number) => (
                      <tr key={`${p.codigo}-${o.tipo}-${o.codigo}-${i}`} className="bg-gray-50/60 dark:bg-gray-800/20 text-xs">
                        ...
                        <td className="px-3 py-1.5 text-gray-600 dark:text-gray-300">{o.nome || '—'}</td>
                        <td className="px-3 py-1.5" />
                        <td className="px-3 py-1.5" />
                        <td className="px-3 py-1.5 text-right tabular-nums text-blue-600 dark:text-blue-400">{o.preco != null ? fmtBRL(o.preco) : <span className="text-amber-500" title="Sem preço nessa origem">—</span>}</td>
                        <td className="px-3 py-1.5 text-right tabular-nums">{o.margem != null ? fmtBRL(o.margem) : '—'}</td>
                        <td className={`px-3 py-1.5 text-right tabular-nums font-bold ${corCmv(o.cmv_pct)}`}>{fmtPct(o.cmv_pct)}</td>
                        {dataAnterior && <td className="px-3 py-1.5" />}
                      </tr>
                    ))}
                  </Fragment>
                  );
