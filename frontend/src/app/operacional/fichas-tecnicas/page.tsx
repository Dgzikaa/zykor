...
import { ChefHat, Plus, Trash2, Search, Boxes, Utensils, Star, Loader2 } from 'lucide-react';
            ...
            <>
              <div className="mb-3 flex flex-wrap items-end justify-between gap-2">
                ...
                <div className="text-right">
                  <div className="text-[11px] text-muted-foreground">Custo da ficha</div>
                  <div className="text-xl font-bold text-emerald-600 dark:text-emerald-400">{fmtBRL(custoTotal)}</div>
                </div>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                    {loadingItens ? <tr><td colSpan={7} className="px-2 py-6 text-center text-gray-400"><Loader2 className="w-5 h-5 animate-spin mx-auto" /></td></tr>
                    : itens.length === 0 ? <tr><td colSpan={7} className="px-2 py-6 text-center text-gray-400">Ficha vazia — adicione os insumos/produções acima.</td></tr>
                    : itens.map(it => (
                      <tr key={it.id} className={it.is_mestre ? 'bg-amber-50/60 dark:bg-amber-900/10' : ''}>
                        ...
                        <td className="px-2 py-1.5 text-right tabular-nums">{fmtPeso(it.quantidade, it.unidade)}</td>
