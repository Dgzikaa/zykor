                ...
                <div>
                  ...
                  {kind === 'produto' && (
                    <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
                      ...
                      {selObj.agrupado_em
                        ? <span className="text-xs text-violet-600 dark:text-violet-300 inline-flex items-center gap-1" title="Usa a ficha (receita/custo) do principal; preço e CMV são os do próprio código ContaHub.">usa a ficha de <b className="font-mono">{selObj.agrupado_em}</b><button onClick={() => salvarGrupo(null)} className="text-gray-400 hover:text-red-500 underline">desagrupar</button></span>
                        : <button onClick={() => { setEditGrupo(true); setGrupoBusca(''); }} className="text-violet-500 hover:text-violet-700 inline-flex items-center gap-1 text-xs" title="Usar a ficha (receita/custo) de um produto principal, mantendo o preço e o CMV deste código. Ideal p/ HH, PagPag, 50%, promoções (mesma receita, preço diferente)."><Boxes className="w-3 h-3" />agrupar (mesma ficha)</button>}
                ...
                {editGrupo && (
                  <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onMouseDown={(e) => { if (e.target === e.currentTarget) setEditGrupo(false); }}>
                    <div className="bg-white dark:bg-gray-900 rounded-xl p-4 w-full max-w-md space-y-2" onClick={e => e.stopPropagation()}>
                      <h4 className="font-semibold text-gray-900 dark:text-white">Agrupar produto (mesma ficha)</h4>
                      <p className="text-sm text-gray-500"><b>{selObj?.nome}</b> passa a usar a <b>ficha (receita/custo) do principal</b> — uma ficha só, editada num lugar. Mas <b>mantém o preço e o CMV do próprio código ContaHub</b>. Ideal p/ Happy Hour, PagPag, 50% e promoções (mesma receita, preço diferente). Dose Dupla <b>não</b> (receita diferente).</p>
                      <div className="relative"><Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" /><Input value={grupoBusca} onChange={e => setGrupoBusca(e.target.value)} placeholder="Buscar o produto principal (a ficha/custo vem dele)…" className="pl-9" /></div>
