...
export default function DrePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [categoriasResponse, dreResponse] = await Promise.all([
        fetch(`/api/financeiro/dre-categorias?ano=${year}&mes=${month}`),
      ...
      const macroCategorias = ordemMacroCategorias.map(macroNome => {
        ...
        // Subcategorias com valores reais do Conta Azul + manuais
        const subcategorias = categoriasDetalhe.map(cat => {
          ...
          if (totalEntradas > 0 || totalSaidas > 0) {
            return { nome: cat.categoria_nome, entradas: totalEntradas, saidas: totalSaidas };
          } else {
            return {
              nome: cat.categoria_nome,
              entradas: isEntrada ? Math.abs(valorMacro) / (categoriasDetalhe.length || 1) : 0,
              saidas: isEntrada ? 0 : Math.abs(valorMacro) / (categoriasDetalhe.length || 1)
            };
          }
        });
  ...
  async function fetchHistoricalData() {
    ...
    try {
      const response = await fetch(`/api/financeiro/dre-yearly-consolidated?year=${year - 1}`);
      if (response.ok) {
        const result = await response.json();
