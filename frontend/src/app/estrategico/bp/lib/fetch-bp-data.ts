...
function calcAnaliseSemanal(
  ...
  const por_dia = DIAS.map(dia => {
    ...
    return {
      dia, eventos_count: evs.length,
      pessoas_real, pessoas_plan,
      tb_real, tb_plan, te_real, te_plan,
      fat_bar_real, fat_bar_plan,
      fat_entrada_real, fat_entrada_plan,
      fat_total_real, fat_total_plan,
      cache_real, cache_plan,
      pct_cache_real, pct_cache_plan,
    };
  });
  ...
  const totais = por_dia.reduce(
    ...
    { eventos_count: 0, pessoas_real: 0, pessoas_plan: 0,
      fat_bar_real: 0, fat_bar_plan: 0, fat_entrada_real: 0, fat_entrada_plan: 0,
      fat_total_real: 0, fat_total_plan: 0, cache_real: 0, cache_plan: 0 }
  );
...
export async function fetchBpData(
  ...
  const [linhasResult, indicadoresResult, versoesResult, eventosResult] = await Promise.all([
    ...
    supabase.from('bp_linha').select('versao, ano').eq('bar_id', barId).eq('ativo', true),
    (supabase as any).schema('operations').from('eventos_base')
      ...
      .eq('bar_id', barId).eq('ativo', true).gte('data_evento', inicio).lte('data_evento', fim),
  ]);
  ...
  for (const l of linhas) {
    if (l.por_dia_semana) bpPorDia[l.linha] = l.por_dia_semana;
  }
  ...
  return { linhas, indicadores, versoes, analise };
}
