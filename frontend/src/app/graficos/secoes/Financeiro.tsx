...
export function SecaoFinanceiro({ barId }: { barId: number; periodo: number }) {
  ...
  const dreMes = useMemo(() => {
    ...
    return Array.from(porMes.entries()).filter(([m]) => fechado(m)).sort((a, b) => a[0] - b[0]).map(([m, o]) => ({
  ...
  const { waterfall, custos, resultadoYtd, receitaYtd } = useMemo(() => {
    ...
    for (const l of dre) { const m = mesNum(l.mes); if (!m || !fechado(m)) continue; const om = Number(l.ordem_macro); acc.set(om, (acc.get(om) || 0) + Number(l.valor_com_sinal || 0)); }
  ...
  const { fluxo, caixa } = useMemo(() => {
    ...
    for (const l of dfc) {
      const m = mesNum(l.mes); if (!m || !fechado(m)) continue;
  ...
  const kpis: Kpi[] = useMemo(() => {
    const u = dreMes[dreMes.length - 1] || { receita: 0, resultado: 0 };
    return [
      ...
      { label: 'CMV / Receita', valor: pct(dreMes.length ? dreMes[dreMes.length - 1].cmv_pct : 0), invLower: true, icon: Boxes },
    ];
  }, [dreMes, receitaYtd, resultadoYtd]);
  ...
  return (
    <div className="space-y-4">
      ...
      <ChartGrid>
        <ChartCard titulo="DRE em cascata (ano, meses fechados)" subtitulo="da Receita ao Resultado Operacional — CMV, custos e despesas" span={2}>
          <GraficoWaterfall passos={waterfall} formatV={moneyK} height={340} />
        </ChartCard>

        <ChartCard titulo="Composição de custos" subtitulo="peso de cada bloco de custo no ano">
