...
| Selecionados p/ pagamento | Diárias já lançadas na semana | Pedidos `tipo=freela` com competência (ou vencimento) dentro de seg→dom | `financial.pedidos_pagamento` |
| Total da semana | Soma das diárias da semana | Σ `valor` das diárias | idem |
| Linha por freela | Nome, nº de diárias, status e total somado | Condensa por pessoa (vínculo CA ou nome) | idem |
