...
4. Ordene mentalmente pela coluna **CMV %** (a cor vermelha indica CMV alto) para achar os itens de pior margem.
...
### Aba Cardápio — tabela de produtos

| Coluna | O que mostra | Como é calculado | Fonte |
|---|---|---|---|
| Cód. | Código interno do produto | direto | `produto_cardapio.codigo` |
| Produto | Nome do produto | direto | `produto_cardapio.nome` |
...
| Preço (CH) | Preço de venda | maior `preco_venda` mapeado no ContaHub; se não houver, cai para o preço Yuzer | `produto_contahub_map` / `gold.produto_preco_yuzer` |
| Margem Bruta | Sobra por unidade | `preco_venda − custo` | calculado |
| CMV % | Custo sobre preço | `custo ÷ preco_venda × 100` (só quando há preço e custo) | calculado |
