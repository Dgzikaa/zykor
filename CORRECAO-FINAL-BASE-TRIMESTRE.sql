-- ============================================================
-- CORREÇÃO FINAL: Mesma lógica de clientes-ativos MAS para TRIMESTRE
-- ============================================================
...
-- Clientes únicos do TRIMESTRE INTEIRO
clientes_unicos AS (
  SELECT bar_id,
         extract(year from dt_gerencial)::int as ano,
         ((extract(month from dt_gerencial)::int - 1) / 3 + 1)::int as trimestre,
...
-- ✅ Base Ativa: Janela de 90 dias que TERMINA no último dia do trimestre
-- Igual a /relatorios/clientes-ativos mas usando fim do trimestre como referência
base_ativa AS (
  SELECT 
    k.bar_id,
    k.ano,
    ...
    (
      ...
      FROM (
        ...
        WHERE cp.bar_id = k.bar_id
          -- 90 dias ANTES do fim do trimestre
          AND cp.dt_gerencial >= (
            LEAST(
              CASE k.trimestre
          ...
          -- ATÉ o fim do trimestre (ou hoje)
        ...
        GROUP BY cli_fone
        -- ✅ MESMA LÓGICA: 2+ DIAS com visitas (não 2+ registros)
        HAVING COUNT(DISTINCT cp.dt_gerencial) >= 2
      ) subq
