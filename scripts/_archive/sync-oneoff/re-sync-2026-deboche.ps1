# Script para re-sincronizar dados de 2026 do ContaHub - DEBOCHE (bar_id = 4)
...
Write-Host "Re-sincronizacao de dados 2026 do ContaHub - DEBOCHE" -ForegroundColor Magenta
...
while ($currentDate -le $endDate) {
    ...
    Write-Host "[$contador/$totalDias - $percentual%] Processando $dataStr..." -ForegroundColor Magenta
    ...
    try {
        $body = @{
            ...
            data_date = $dataStr
            automated = $false
            source = "manual-resync-2026-deboche"
        } | ConvertTo-Json
    ...
    # Aguardar 2 segundos entre requisicoes
...
Write-Host ""
Write-Host "====================================================" -ForegroundColor Magenta
Write-Host "Re-sincronizacao DEBOCHE concluida!" -ForegroundColor Magenta
