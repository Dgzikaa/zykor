// Script para sincronizar DEBOCHE (bar_id=4) - 2025 COMPLETO
...
async function getSyncedDays() {
  ...
  try {
    const response = await fetch(`${SUPABASE_URL}/rest/v1/contahub_raw_data?bar_id=eq.${BAR_ID}&data_date=gte.2025-01-01&data_date=lte.2025-12-31&select=data_date`, {
      headers: {
        'apikey': SERVICE_KEY,
...
async function syncDeboche2025() {
  const startDate = new Date(Date.UTC(2025, 0, 1)); // 01/01/2025
  const endDate = new Date(Date.UTC(2025, 11, 31)); // 31/12/2025
  ...
  console.log('\x1b[36m🚀 Iniciando sincronização DEBOCHE 2025 (bar_id=' + BAR_ID + ')\x1b[0m');
  console.log('\x1b[36m📅 Período: 01/01/2025 a 31/12/2025\x1b[0m');
...
syncDeboche2025().catch(console.error);
