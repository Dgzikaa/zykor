# Script para processar todos os dados raw de tempo de 2026
...
Write-Host "PROCESSAR RAW DATA - TEMPO 2026" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# Buscar todos os IDs de raw_data não processados de tempo
...
try {
    $response = Invoke-RestMethod -Uri "$supabaseUrl/rest/v1/contahub_raw_data?select=id,data_date&data_type=eq.tempo&processed=eq.false&order=data_date.asc" -Headers $headers -Method Get
