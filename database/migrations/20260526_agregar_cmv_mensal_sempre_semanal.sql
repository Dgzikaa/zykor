-- Aplicada via MCP em 2026-05-26.
-- Resolve task #75: estoque_final do CMV Mensal pegando domingo errado.
--
-- Bug: a versao anterior preservava estoque_inicial/final da planilha mensal
-- quando fonte IN ('planilha', 'mista', 'manual'). Como sync-cmv-mensal escrevia
-- como 'planilha'/'mista', o agregador NUNCA atualizava o estoque mesmo quando
-- a cmv_semanal (planilha SEMANAL) tinha valores mais recentes.
--
-- Caso real (Mai/26 Ord):
--   Antes: est_ini=234.243,38 / est_fim=175.679,94 (lixo da planilha mensal)
--   Depois: est_ini=207.372,88 (S19) / est_fim=178.174,38 (S21 = ultimo domingo)
--
-- Agora estoque sempre deriva de financial.cmv_semanal (planilha SEMANAL).
-- Bonificacoes e ajuste_bonif continuam preservados (sao manuais).
...
BEGIN
  -- Preservar APENAS os campos genuinamente manuais (bonificacoes, ajuste_bonif).
  -- Estoques agora vem SEMPRE de cmv_semanal.
  SELECT fonte,
  ...
  -- Estoques CMV: SEMPRE de cmv_semanal (planilha semanal = fonte unica).
  -- est_ini = primeira semana cuja quinta cai no mes.
  -- est_fim = ultima semana cuja quinta cai no mes COM estoque_final > 0.
