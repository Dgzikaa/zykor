-- 2026-06-14 — Estoque mensal manual: agregar_cmv_mensal_auto preserva estoque quando
-- fonte IN ('planilha','manual'). O PUT /api/cmv-semanal/mensal marca fonte='manual' ao
-- editar estoque na tela; sem isso, o próximo recálculo derivava do semanal e apagava a
-- edição manual. Mantém o label 'manual' (não rebaixa pra 'planilha').
--
-- Demais regras idênticas a 20260614_cmv_mensal_compras_bronze_competencia.sql
-- (compras por competência/bronze, bonificações somam).
...
BEGIN
  ...
  -- Estoque manual/planilha: preservar quando já gravado (fonte planilha OU manual e > 0).
