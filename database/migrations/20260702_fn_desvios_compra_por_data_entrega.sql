-- Desvio de consumo: contar a COMPRA pela data de ENTREGA (dt_entrega), não a do pedido.
--
-- Bug reportado (30/06, Xeque Mate 355ml Lata, bar 3, pedido 3896004): o pedido foi feito em
-- 30/06 mas entregue em 01/07 (240 latas). O desvio de 30/06 contava as 240 como compra do dia
-- (data do pedido), inflando o estoque teórico e marcando -240 de perda falsa.
--
-- Correção: no CTE `compras` de gold.fn_desvios e gold.fn_desvios_proteina, filtrar por
-- coalesce(pp.dt_entrega, pp.data). A compra entra no período em que a mercadoria FISICAMENTE
-- chegou (entrega). Fallback pra data do pedido só quando não há dt_entrega registrado — o
-- VMarket só passou a preencher dt_entrega em 27/03/2026, e as contagens/desvios existem desde
-- 2025; sem o fallback, todo o histórico anterior perderia as compras. Todo pedido com status
-- "Entrega Confirmada" (id_pedido_status=6) tem dt_entrega preenchido, então pedidos entregues
-- caem sempre na data certa; pedidos ainda não entregues (dt_entrega NULL) usam a data do pedido
-- e se auto-corrigem quando a entrega é confirmada (a função é STABLE, recalcula ao vivo).
...
AS $function$
  ...
  -- compra entra pela data de ENTREGA (fallback: data do pedido quando não há dt_entrega registrado)
  compras as (select upper(coalesce(b.codigo_planilha,b.cod_interno)) as cod, sum(pi.quantidade) as qtd
    ...
    where pi.bar_id=p_bar and coalesce(pp.dt_entrega, pp.data) >= p_ini and coalesce(pp.dt_entrega, pp.data) < p_fim group by 1),
