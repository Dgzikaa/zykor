-- fn_plano_compras: consumo por vendas passa a EXPANDIR a ficha recursivamente
-- (prato de cardápio -> preparo(s)/produção -> insumo), escalando por rendimento.
--
-- Bug: o `saida` (consumo histórico que alimenta média/PR) só somava os insumos DIRETOS da ficha
-- do prato vendido. Insumo usado apenas dentro de um PREPARO (ex.: Farinha Panko i0240 na Coxinha/
-- Empanação) ficava com consumo 0 -> o insumo caía em "não comprar" mesmo sendo muito usado, e sumia
-- do filtro "A comprar" ("aparece como se não estivesse cadastrado"). No Deboche, 83 insumos ficavam
-- com consumo zerado por esse motivo.
--
-- Fix: CTE recursiva `tree` desce do produto pela árvore da ficha. qtd_eff = qtd/fator_correcao *
-- multiplicador; ao entrar num componente 'producao', escala por (qtd_eff / rendimento_do_preparo) *
-- (qtd_do_insumo / fc). Isso alinha o consumo por vendas à semântica oficial de silver.insumo_por_produto
-- e ainda cobre os insumos de preparo. Guarda de profundidade (depth < 8) evita loop em ciclo de ficha.
--
-- Validação (Deboche, semana 2026-07-20): panko i0240 saída 6 semanas 0 -> ~3.219 g
-- (~573 g/semana); insumos com saída passaram de 140 -> 223 (83 novos, todos de preparo).
-- AB (necessidade do plano encerrado da semana) segue como antes, no nível da produção planejada.
-- Já aplicado em produção.

CREATE OR REPLACE FUNCTION gold.fn_plano_compras(p_bar integer, p_semana date DEFAULT NULL::date)
 RETURNS TABLE(insumo_codigo text, nome text, fornecedor text, categoria text, secao_vmarket text, unidade_medida text, base text, embalagem numeric, custo numeric, curva_a boolean, estoque_cont numeric, ab numeric, comprado numeric, semanas date[], saidas numeric[])
...
AS $function$
  ...
  direta as (
    select produto_id, cod, sum(qtd_eff) qtd
    from tree where componente_tipo='insumo' and cod is not null
    group by produto_id, cod
  ),
