-- fn_desvios integrado ao Controle de Produção (operations.producao_execucao*):
--  * INSUMO saída teórica = consumo direto em produto (matview) + consumo REAL do Controle (qtd_real)
--                           + estimativa por produzido manual (só p/ produções SEM execução no Controle).
--  * PRODUÇÃO "Produzido" = rendimento_real do Controle / fator_contagem (fallback: produzido manual).
--  * Exclui linhas pc/pd do consumo real (produção-componente, tratada no balanço da produção).
--  * Embalagem segue de insumo_unidade (não usar operations.insumos: valores errados p/ cerveja = R$19M de sobra).
...
AS $function$
  ...
  exec_no_periodo as (
    select distinct producao_id from operations.producao_execucao
    where bar_id=p_bar and inicio::date >= p_ini and inicio::date < p_fim
  ),
  exec_rend as (
    select pe.producao_id, sum(pe.rendimento_real) rr
    from operations.producao_execucao pe
    where pe.bar_id=p_bar and pe.inicio::date >= p_ini and pe.inicio::date < p_fim and pe.rendimento_real is not null
    group by pe.producao_id
  ),
  real_prod as (
    select upper(pei.insumo_codigo) cod, sum(pei.qtd_real) as base
    ...
    where pe.bar_id=p_bar and pe.inicio::date >= p_ini and pe.inicio::date < p_fim
      and pei.qtd_real is not null and pei.insumo_codigo is not null and pei.insumo_codigo !~* '^p[cd]'
  ...
  entrada as (
    select pb.cod, coalesce(er.rr / nullif(pb.fator_contagem,0), em.qtd) as qtd
    from prod_base pb
    left join exec_rend er on er.producao_id=pb.id
    left join entrada_manual em on em.cod=pb.cod
    where er.rr is not null or em.qtd is not null
  ),
  teorico_ins_prod as (
    select pim.cod, sum(em.qtd * pb.fator_contagem * pim.q) as base
    from entrada_manual em
    join prod_base pb on pb.cod = em.cod and pb.entra_contagem
    join prod_insumo_mult pim on pim.rcod = em.cod
    where not exists (select 1 from exec_no_periodo x where x.producao_id = pb.id)
    group by pim.cod
  ),
  teorico_prod as (
  ...
  joined as (
    ...
    left join real_prod rp on rp.cod=k.cod
