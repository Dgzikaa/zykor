-- agregar_cmv_mensal_auto: Faturamento Limpo (faturamento_cmvivel) dos meses FECHADOS
-- passa a descontar a bilheteria (Sympla + ingresso Yuzer), igual ja faz a rota do mes
-- corrente (frontend/src/app/api/cmv-semanal/mensal/route.ts). Antes so tirava
-- comissao + couvert, entao em mes com evento Yuzer/Sympla o ingresso inflava o Fat
-- Limpo e o denominador do CMV% (ex.: bar 4 jun/2026 501.485,62 -> 431.872,39).
--
-- Fat Limpo = faturamento_total_consolidado - comissao - couvert - (sympla_liquido + faturamento_entrada_yuzer)
--
-- Espelha o desconto de ingresso Yuzer ja aplicado no semanal (cmv-semanal-auto).
-- Reprocessado 2026 apos o deploy; 2025 nao foi tocado (cmv_mensal/semanal de 2025
-- tem base diferente/incompleta — ver feedback_mutacao_sql_base_por_periodo).
...
BEGIN
  ...
  -- Faturamento bruto (consolidado, inclui Yuzer/Sympla) + bilheteria a descontar do limpo.
  ...
  -- Fat Limpo = bruto - comissao - couvert - bilheteria (ingresso Sympla/Yuzer nao e' F&B).
