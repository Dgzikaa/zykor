-- Embalagem do desvio passa a usar a MESMA fonte da tela de Insumos:
--   valor salvo (insumo_unidade) e, quando ausente, DERIVA do nome (operations.derive_embalagem),
--   espelhando lib/insumo-unidade.ts (deriveUnid). Antes o desvio lia só insumo_unidade -> itens sem
--   esse registro (cadastro de unidade feito no Zykor) zeravam a saída teórica = perda falsa.
-- NÃO usar operations.insumos.embalagem (tem lixo, ex.: cerveja=1 -> causou R$19M de sobra falsa).

CREATE OR REPLACE FUNCTION operations.derive_embalagem(p_nome text, p_um text)
RETURNS numeric LANGUAGE plpgsql IMMUTABLE AS $$
declare n text := lower(coalesce(p_nome,'')); um text := lower(btrim(coalesce(p_um,''))); m text[]; num numeric;
begin
  m := regexp_match(n, '(\d+[.,]?\d*)\s*(kg|kilo|litro|lt|ml|gr|grama|l|g)\y');
  if m is not null then
    num := replace(replace(m[1],'.',''),',','.')::numeric;
    if m[2] in ('kg','kilo') then return num*1000; end if;
    if m[2] in ('l','lt','litro') then return num*1000; end if;
    if m[2]='ml' then return num; end if;
    if m[2] in ('g','gr','grama') then return num; end if;
  end if;
  m := coalesce(regexp_match(n,'c/\s*(\d+)'), regexp_match(n,'(\d+)\s*(und|unid|cx|caixa|pct|pacote|fardo)\y'));
  if m is not null then return coalesce(m[1]::numeric,1); end if;
  if n ~ 'vinho|espumante|frisante|moscatel|prosecco|sparkling|(^|\s)v\.|(^|\s)esp\.' then return 750; end if;
  if n ~ 'whisky|vodka|\ygin\y|tequila|cacha|\yrum\y|licor|conhaque|brandy|aperol|campari|cynar|vermouth|jager|bitter|absinto|steinha|amarula|cointreau|frangelico|limoncello|domecq|netuno|presidente|bananinha|\yjambu\y' then return 1000; end if;
  if um in ('ml','l','litro') then return 1000; end if;
  if um in ('kg','g','grama') then return 1000; end if;
  return 1;
end $$;

-- fn_desvios: preco_emb usa coalesce(insumo_unidade.embalagem, operations.derive_embalagem(nome, unidade)).
-- (corpo completo — versão vigente com Controle de Produção + entra_contagem + explosão)
CREATE OR REPLACE FUNCTION gold.fn_desvios(p_bar integer, p_ini date, p_fim date)
 ...
 LANGUAGE sql STABLE SET search_path TO 'gold', 'public', 'operations', 'silver'
AS $function$
  with recursive
  expl as (
