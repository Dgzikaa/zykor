-- fn_desvios: decompõe pré-batch (producao_base.decompor_contagem) contado nos insumos componentes,
-- somando em est_ini/est_fim deles. Fórmula por insumo componente:
--   estoque_prebatch(unid contagem) × fator_contagem(prebatch) × (insumo por base do prebatch) / embalagem(insumo).
-- Usa a MESMA embalagem (insumo_unidade → derive) que o resto do desvio, pra ficar consistente com est cru e saída.
-- Versão vigente: Controle de Produção (consumo real/produzido) + entra_contagem + embalagem-deriva + pré-batch.
