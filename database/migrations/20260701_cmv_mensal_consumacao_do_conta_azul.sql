-- CMV mensal: consumação passa a vir do CONTA AZUL (categorias [Consumação] X), não mais
-- do ContaHub × fator. Motivo: no CA a consumação foi classificada manualmente por categoria
-- (mesma base da DRE), já em CUSTO. A soma das [Consumação] X = o contra-lançamento agregado
-- '[CONSUMAÇÃO] AJUSTE CMV' (validado: bar 3 2026 = R$ 152.268 nas categorias = R$ 152.269 no ajuste).
-- Assim o mensal casa com a DRE, sem "outros", distribuído por mês.
--
-- Só o bloco de consumação da agregar_cmv_mensal_auto muda; o resto é idêntico.
-- Reprocessar 2026 após aplicar:
--   do $$ declare b int; m int; begin
--     foreach b in array array[3,4] loop for m in 1..12 loop
--       perform public.agregar_cmv_mensal_auto(b, 2026, m); end loop; end loop; end $$;
...
BEGIN
  ...
  -- Consumação = CUSTO real do Conta Azul (categorias [Consumação] X). Não multiplica por fator.
  -- Exclui '[CONSUMAÇÃO] AJUSTE CMV' (contra-lançamento agregado) pra não cancelar a soma.
  SELECT
    COALESCE(SUM(v) FILTER (WHERE cat ILIKE '%sócios%' OR cat ILIKE '%socios%'), 0),
    COALESCE(SUM(v) FILTER (WHERE cat ILIKE '%cliente%' OR cat ILIKE '%aniversár%' OR cat ILIKE '%aniversar%' OR cat ILIKE '%influencer%' OR cat ILIKE '%pontos%'), 0),
