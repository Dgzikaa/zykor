-- Pré-batch (Opção B do Gonza): produção que, ao ser CONTADA, decompõe a quantidade nos insumos
-- que a compõem e soma no estoque cru deles (importante pro DESVIO — quantidade, não R$).
-- Ex.: 10 L de Pré-Batch Negroni contados → +3,33 L de gin/vermouth/campari no est de cada.
-- Flag por produção (checkbox na ficha). fn_desvios decompõe est_ini/est_fim; contagem_itens torna o pré-batch contável.

alter table public.producao_base add column if not exists decompor_contagem boolean not null default false;
comment on column public.producao_base.decompor_contagem is
  'Pré-batch: ao contar, explode a quantidade nos insumos componentes (soma no estoque cru). Sem SKU proprio.';

-- contagem_itens: pré-batches (decompor_contagem) passam a ser contáveis (aparecem na contagem).
CREATE OR REPLACE FUNCTION operations.contagem_itens(p_bar_id integer, p_tipo text, p_data date)
 ...
 LANGUAGE sql STABLE SET search_path TO 'operations', 'public', 'pg_catalog'
AS $function$
  ...
  where pb.bar_id=p_bar_id and pb.codigo is not null and coalesce(pb.ativo,true)
    and ( coalesce(pb.entra_contagem, true) or coalesce(pb.decompor_contagem, false) )
    and ( p_tipo in ('mensal','semanal') or (p_tipo='diaria' and pb.curva_a=true) )
  order by 5, 4 nulls last, 3;
$function$;

-- NOTA: gold.fn_desvios foi atualizada na mesma leva (migration 20260630_fn_desvios_decompor_prebatch)
-- pra somar a decomposição do pré-batch em est_ini/est_fim dos insumos componentes.
