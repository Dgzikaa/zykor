-- Corrigir extração de hora e mes nas funções

-- 1. CORRIGIR process_fatporhora_data (extrair hora de "16:00" -> 16)
...
-- 2. CORRIGIR process_tempo_data (extrair mes de "2026-03" -> 3)
...
BEGIN
    FOR item_json IN SELECT jsonb_array_elements(p_data_array) LOOP
        -- Extrair mes de formato "2026-03" -> 3
        mes_str := item_json->>'mes';
        mes_int := CASE 
            WHEN mes_str IS NULL THEN 0
            WHEN mes_str ~ '^\d{4}-\d{2}$' THEN SPLIT_PART(mes_str, '-', 2)::integer
            ELSE COALESCE(mes_str::integer, 0)
        END;
        ...
        INSERT INTO contahub_tempo (
            ...
            COALESCE((item_json->>'prd')::integer, 0),
