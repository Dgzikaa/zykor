-- Plano de Compras: unidade/embalagem vêm da MESMA fonte da tela de Insumos.
-- Bug corrigido (29/06): a função lia operations.insumos.embalagem (TEXT cru, na maioria 1/null),
-- enquanto a tela de Insumos usa silver.insumo_catalogo (base/embalagem, vindo de insumo_unidade)
-- com fallback derivado do NOME. Resultado: números gigantes em ml/g e "emb." sem conversão.
-- Agora a função devolve base/embalagem do catálogo + estoque CRU (contagem em pacotes);
-- a resolução final da unidade (catálogo → fallback deriveUnid pelo nome) e toda a conta de
-- base/embalagem acontecem na rota (lib compartilhada src/lib/insumo-unidade.ts), igual à tela.
...
create or replace function gold.fn_plano_compras(p_bar integer, p_semana date default null)
 returns table(insumo_codigo text, nome text, fornecedor text, categoria text,
...
as $function$
  ...
  full_grid as (
    select i.cod, i.nome, i.fornecedor, i.categoria, i.unidade_medida, i.custo_unitario, i.curva_a,
  ...
  select fg.cod, fg.nome::text, fg.fornecedor::text, fg.categoria::text,
