-- Corrigir funções process_tempo_data e process_fatporhora_data
-- Remover coluna 'semana' que não existe no banco

-- 1. CORRIGIR process_fatporhora_data
...
BEGIN
    FOR item_json IN SELECT jsonb_array_elements(p_data_array) LOOP
        INSERT INTO contahub_tempo (
            ...
            COALESCE(item_json->>'ano', '')::integer,
            COALESCE(item_json->>'mes', '')::integer,
            COALESCE(item_json->>'dia', '')::timestamp,
            COALESCE((item_json->>'dds')::integer, 0),
