-- fn_plano_producao: a saída de cada semana deixa de arredondar pra 2 casas
-- (round(saida,2) -> round(saida,6)) p/ a média ser exata. Display continua 2 casas.
-- Não fecha a diferença vs planilha do sócio nas camadas profundas (carne/massa): essa
-- diferença é CORTESIA — a planilha usa venda (sem cortesia) na explosão das camadas, o
-- Zykor usa consumo (com cortesia). Zykor é o consistente/correto (cortesia consome insumo).
...
as $function$
  ...
  prod_base as (
    select id, upper(codigo) cod, nome, coalesce(rendimento,0) rendimento, coalesce(fator_contagem,1) fator_contagem,
           coalesce(nullif(unidade_contagem,''),
             case when coalesce(fator_contagem,1)=1 then unidade
                  ...
                  else unidade end) as unidade_contagem,
