-- BUG GRAVE (Isaías, 16/07): a troca entre bares lançava o insumo no bar destino usando o
-- código do bar de ORIGEM. Os códigos i0XXX são independentes por bar — dos 283 códigos que
-- existem nos dois bares, 281 são insumos DIFERENTES. Resultado: o Deboche emprestou 60 pães
-- Smash (i0279) e o desvio do Ordinário acusou -60 de "Espumante Unus Merlot Rosé" (i0279 lá).
-- As 3 trocas já registradas estavam 100% erradas (5/5 itens em insumo trocado).
--
-- FIX: o item da troca passa a guardar OS DOIS códigos — o do emissor (insumo_codigo, que já
-- existia) e o equivalente no bar recebedor (insumo_codigo_destino, novo). A perna de SAÍDA
-- usa o código de origem; a de ENTRADA usa o de destino. Sem equivalente (NULL) → a entrada
-- não é lançada no destino (a saída no emissor continua valendo), em vez de cair em insumo errado.

alter table financial.troca_itens
  add column if not exists insumo_codigo_destino text;

comment on column financial.troca_itens.insumo_codigo   is 'Código do insumo no catálogo do bar_origem (quem enviou) — perna de SAÍDA do desvio.';
comment on column financial.troca_itens.insumo_codigo_destino is 'Código EQUIVALENTE no catálogo do bar_destino — perna de ENTRADA do desvio. NULL = sem equivalente cadastrado no destino (entrada não é lançada).';

-- Backfill das 3 trocas existentes (todas Deboche bar 4 → Ordinário bar 3), de-para confirmado
-- com o Rodrigo em 16/07. i0274 "Pão French bread" → i0141 "Pão Francês und" (Ordi não tem
-- French bread); i0198 "Blend HB 100" → i0493 "Blend APF 100" (mesmo blend, nome de fornecedor).
update financial.troca_itens ti
set insumo_codigo_destino = m.cod_destino
from financial.trocas t,
     (values ('i0279','i0508'),   -- Pão Smash classic  → Pão Smash und
             ('i0278','i0507'),   -- Pão Pretzel        → Pão Pretzel und
             ('i0274','i0141'),   -- Pão French bread   → Pão Francês und
             ('i0198','i0493')    -- Blend HB 100 (kg)  → Blend APF 100 (kg)
     ) as m(cod_origem, cod_destino)
where ti.troca_id = t.id
  and t.bar_origem = 4 and t.bar_destino = 3
  and lower(ti.insumo_codigo) = m.cod_origem
  and ti.insumo_codigo_destino is null;


-- ============================================================================
-- gold.fn_desvios — igual à versão de 20260709 (rótulo unidade_contagem);
-- ÚNICA mudança: troca_ent passa a casar por insumo_codigo_destino.
-- ============================================================================
...
AS $function$
  ...
  -- ENTRADA: casa pelo código do insumo NESTE bar (o equivalente escolhido por quem enviou).
  -- Sem equivalente (NULL) a entrada não é lançada — melhor faltar do que cair em insumo errado.
  troca_ent as (select upper(ti.insumo_codigo_destino) cod, sum(ti.quantidade) qtd
    from financial.trocas t join financial.troca_itens ti on ti.troca_id=t.id
    where t.bar_destino=p_bar and t.status <> 'cancelada' and ti.insumo_codigo_destino is not null
      and t.data_competencia >= p_ini and t.data_competencia < p_fim group by 1),
  -- SAÍDA: código do próprio emissor.
...
-- ============================================================================
-- gold.fn_desvios_proteina — igual à versão de 20260707; ÚNICA mudança: troca_ent.
-- ============================================================================
CREATE OR REPLACE FUNCTION gold.fn_desvios_proteina(p_bar integer, p_ini date, p_fim date)
 RETURNS TABLE(insumo_cod text, insumo_nome text, unidade text, estoque_ini numeric, comprou numeric, troca numeric, utilizado_producao numeric, saida_direta numeric, desperdicio numeric, estoque_fim_teorico numeric, estoque_fim_real numeric, desvio_qtd numeric, preco numeric, desvio_rs numeric)
 LANGUAGE sql
...
AS $function$
  ...
  -- ENTRADA pelo código DESTE bar (equivalente escolhido por quem enviou); NULL = não lança.
  troca_ent as (select upper(ti.insumo_codigo_destino) cod, sum(ti.quantidade) qtd
    from financial.trocas t join financial.troca_itens ti on ti.troca_id=t.id
    where t.bar_destino=p_bar and t.status <> 'cancelada' and ti.insumo_codigo_destino is not null
      and t.data_competencia >= p_ini and t.data_competencia < p_fim group by 1),
  ...
  select pr.codigo, pr.nome::text, 'kg'::text,
    ...
    coalesce(te.qtd,0) - coalesce(ts.qtd,0) as troca,
    round(coalesce(um.q, uc.q, ua.q, 0),3) as utilizado_producao,
    round(coalesce(sd.q,0),3) as saida_direta,
    coalesce(dp.qtd,0),
    round(coalesce(ei.q,0)+coalesce(c.q,0)+(coalesce(te.qtd,0)-coalesce(ts.qtd,0))-coalesce(um.q, uc.q, ua.q, 0)-coalesce(sd.q,0)-coalesce(dp.qtd,0),3) as estoque_fim_teorico,
    coalesce(ef.q,0),
    round(coalesce(ef.q,0) - (coalesce(ei.q,0)+coalesce(c.q,0)+(coalesce(te.qtd,0)-coalesce(ts.qtd,0))-coalesce(um.q, uc.q, ua.q, 0)-coalesce(sd.q,0)-coalesce(dp.qtd,0)),3) as desvio_qtd,
    round(p.preco,2),
    round((coalesce(ef.q,0) - (coalesce(ei.q,0)+coalesce(c.q,0)+(coalesce(te.qtd,0)-coalesce(ts.qtd,0))-coalesce(um.q, uc.q, ua.q, 0)-coalesce(sd.q,0)-coalesce(dp.qtd,0))) * coalesce(p.preco,0),2) as desvio_rs
  from prot pr
  ...
  left join troca_ent te on te.cod=upper(pr.codigo)
  left join troca_sai ts on ts.cod=upper(pr.codigo)
