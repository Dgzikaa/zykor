-- 2026-06-14 — CMV mensal: Compras por COMPETÊNCIA (pagas + a pagar), fonte bronze.
-- Decisão do sócio: o semanal (correto) usa a base completa; o mensal usava silver
-- (só lançamentos pagos/ACQUITTED), ficando ~42k abaixo e divergindo do detalhamento
-- exibido na tela (que sempre veio do bronze). Resultado: a "soma das contas" do
-- tooltip nunca batia com o CMV R$.
--
-- Agora a função usa EXATAMENTE a mesma lógica de Compras da rota
-- /api/cmv-semanal/mensal (bronze.bronze_contaazul_lancamentos, valor efetivo
-- = pago>0?pago:bruto, RECEITA subtrai, excluido_em IS NULL). Assim headline = soma
-- do detalhamento, e mensal = semanal.
--
-- compras_alimentacao (CMA) segue do silver/bruto p/ continuar batendo com a planilha
-- (bloco CMA é separado do CMV e não foi alvo desta correção).
-- Supersede o bloco de compras de 20260614_cmv_mensal_bonif_soma.sql.
...
BEGIN
  ...
  -- Compras CMV: COMPETÊNCIA (pagas + a pagar) via bronze — mesma lógica da rota mensal.
  -- valor efetivo (pago>0?pago:bruto); RECEITA subtrai (devoluções reduzem custo).
  SELECT
  ...
  -- CMA (alimentação funcionários): segue do silver/bruto p/ bater com a planilha.
  ...
  -- bonificações SOMAM (igual ao semanal); compras por competência (bronze).
