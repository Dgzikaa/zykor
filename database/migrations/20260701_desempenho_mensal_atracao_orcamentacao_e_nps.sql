...
BEGIN
  ...
  -- Cutoffs historicos (data_producao >= cutoff -> usa t0_t3, senao t0_t2)
  IF p_bar_id = 3 THEN
    ...
    v_cut_coz := DATE '9999-12-31'; -- Ord cozinha SEMPRE t0_t2
    v_cut_bar := DATE '1900-01-01'; -- Ord bar    SEMPRE t0_t3
  ELSIF p_bar_id = 4 THEN
    v_locs_drinks := ARRAY['Bar'];
    v_locs_cozinha := ARRAY['Cozinha','Cozinha 2'];
    v_cut_coz := DATE '2026-03-20'; -- Deb cozinha: t0_t3 a partir de 20/03
    v_cut_bar := DATE '2026-03-07'; -- Deb bar:     t0_t3 a partir de 07/03
  ELSE
    v_locs_drinks := NULL; v_locs_cozinha := NULL;
    v_cut_coz := DATE '9999-12-31'; v_cut_bar := DATE '9999-12-31';
  END IF;
  ...
  WITH fase_planejamento AS (
    SELECT
      ...
      CASE WHEN SUM(faturamento_total_consolidado) > 0
        THEN (SUM(descontos)::numeric / SUM(faturamento_total_consolidado) * 100)::numeric(5,2) ELSE 0 END as desconto_percentual,
      ...
      (COALESCE(AVG(faturamento_total_consolidado) FILTER (WHERE EXTRACT(dow FROM data_evento)=4),0)
       + COALESCE(AVG(faturamento_total_consolidado) FILTER (WHERE EXTRACT(dow FROM data_evento)=6),0)
       + COALESCE(AVG(faturamento_total_consolidado) FILTER (WHERE EXTRACT(dow FROM data_evento)=0),0))::numeric(14,2) as qui_sab_dom,
      (COALESCE(AVG(faturamento_total_consolidado) FILTER (WHERE EXTRACT(dow FROM data_evento)=2),0)
       + COALESCE(AVG(faturamento_total_consolidado) FILTER (WHERE EXTRACT(dow FROM data_evento)=3),0)
       + COALESCE(AVG(faturamento_total_consolidado) FILTER (WHERE EXTRACT(dow FROM data_evento)=4),0))::numeric(14,2) as ter_qua_qui,
      (COALESCE(AVG(faturamento_total_consolidado) FILTER (WHERE EXTRACT(dow FROM data_evento)=5),0)
       + COALESCE(AVG(faturamento_total_consolidado) FILTER (WHERE EXTRACT(dow FROM data_evento)=6),0))::numeric(14,2) as sex_sab
    FROM gold.planejamento
    WHERE planejamento.bar_id = p_bar_id AND data_evento BETWEEN v_data_inicio AND v_data_fim
  ),
  ...
  fase_clientes AS (
    SELECT
      ...
      (SELECT CASE WHEN SUM(total_clientes_unicos_dia) > 0
        THEN (SUM(novos_clientes_dia)::numeric / SUM(total_clientes_unicos_dia) * 100)::numeric(5,2) END
  ...
  fase_nps_digital AS (
    ...
    FROM bronze.bronze_falae_respostas
    WHERE bar_id = p_bar_id AND (created_at AT TIME ZONE 'America/Sao_Paulo')::date BETWEEN v_data_inicio AND v_data_fim AND search_name = 'NPS'
  ),
  fase_nps_salao AS (
    ...
    SELECT
      CASE WHEN SUM((respostas_por_pesquisa->'(reserva getin)'->>'total')::integer) > 0
        THEN (SUM((respostas_por_pesquisa->'(reserva getin)'->>'total')::integer * (respostas_por_pesquisa->'(reserva getin)'->>'nps_medio')::numeric)
             / SUM((respostas_por_pesquisa->'(reserva getin)'->>'total')::integer))::numeric(5,2) END as nps_reservas,
  ...
  fase_tempos_agg AS (
    SELECT
      ...
      CASE WHEN COUNT(*) FILTER (WHERE categoria = 'bebida' AND tempo_bar_efetivo > 0) > 0
        THEN AVG(tempo_bar_efetivo) FILTER (WHERE categoria = 'bebida' AND tempo_bar_efetivo > 0)::numeric(8,2) END as tempo_bebidas,
      ...
      COUNT(*) FILTER (WHERE local_desc = ANY(v_locs_drinks) AND tempo_bar_efetivo > 600)::integer as atrasao_drinks,
      COUNT(*) FILTER (WHERE local_desc = ANY(v_locs_cozinha) AND tempo_coz_efetivo BETWEEN 900 AND 1200)::integer as atrasinho_cozinha,
      COUNT(*) FILTER (WHERE local_desc = ANY(v_locs_cozinha) AND tempo_coz_efetivo > 1200)::integer as atrasao_cozinha,
      COUNT(*) FILTER (WHERE local_desc = ANY(v_locs_drinks) AND tempo_bar_efetivo > 600)::integer as atrasao_bar
  ...
  fase_stockout AS (SELECT * FROM silver.calcular_stockout_periodo(p_bar_id, v_data_inicio, v_data_fim)),
  fase_google AS (
    SELECT
  ...
  fase_custos AS (
    SELECT
      COALESCE(SUM(COALESCE(NULLIF(valor_pago,0), valor_bruto)) FILTER (WHERE categoria_nome ILIKE '%atra%' OR categoria_nome ILIKE '%produ%evento%'), 0)::numeric(14,2) as atracoes_eventos,
      COALESCE(SUM(COALESCE(NULLIF(valor_pago,0), valor_bruto)) FILTER (WHERE categoria_nome ILIKE '%freela%'), 0)::numeric(14,2) as freelas,
      COALESCE(SUM(COALESCE(NULLIF(valor_pago,0), valor_bruto)) FILTER (WHERE categoria_nome ILIKE '%material%' OR categoria_nome ILIKE '%limpeza%'), 0)::numeric(14,2) as materiais,
      COALESCE(SUM(COALESCE(NULLIF(valor_pago,0), valor_bruto)) FILTER (WHERE categoria_nome ILIKE '%comiss%'), 0)::numeric(14,2) as comissao,
      COALESCE(SUM(COALESCE(NULLIF(valor_pago,0), valor_bruto)) FILTER (WHERE categoria_nome ILIKE '%sal%rio%' OR categoria_nome ILIKE '%vale%transp%'), 0)::numeric(14,2) as cmo,
      COALESCE(SUM(COALESCE(NULLIF(valor_pago,0), valor_bruto)) FILTER (WHERE categoria_nome ILIKE '%pro%labore%'), 0)::numeric(14,2) as pro_labore,
      COALESCE(SUM(COALESCE(NULLIF(valor_pago,0), valor_bruto)) FILTER (WHERE categoria_nome ILIKE '%aluguel%' OR categoria_nome ILIKE '%iptu%' OR categoria_nome ILIKE '%condom%' OR categoria_nome ILIKE '%%gua%' OR categoria_nome ILIKE '%luz%'), 0)::numeric(14,2) as ocupacao_custo,
      COALESCE(SUM(COALESCE(NULLIF(valor_pago,0), valor_bruto)) FILTER (WHERE categoria_nome ILIKE '%imposto%'), 0)::numeric(14,2) as imposto,
      COALESCE(SUM(COALESCE(NULLIF(valor_pago,0), valor_bruto)) FILTER (WHERE categoria_nome ILIKE '%escrit%central%'), 0)::numeric(14,2) as escritorio_central,
      COALESCE(SUM(COALESCE(NULLIF(valor_pago,0), valor_bruto)) FILTER (WHERE categoria_nome ILIKE '%marketing%'), 0)::numeric(14,2) as marketing_fixo,
      COALESCE(SUM(COALESCE(NULLIF(valor_pago,0), valor_bruto)) FILTER (WHERE categoria_nome ILIKE '%taxa%maquin%'), 0)::numeric(14,2) as adm_fixo,
      COALESCE(SUM(COALESCE(NULLIF(valor_pago,0), valor_bruto)) FILTER (WHERE categoria_nome ILIKE '%manuten%'), 0)::numeric(14,2) as manutencao,
      COALESCE(SUM(COALESCE(NULLIF(valor_pago,0), valor_bruto)) FILTER (WHERE categoria_nome ILIKE '%utens%'), 0)::numeric(14,2) as utensilios,
      COALESCE(SUM(COALESCE(NULLIF(valor_pago,0), valor_bruto)) FILTER (WHERE categoria_nome ILIKE '%alimenta%'), 0)::numeric(14,2) as alimentacao
  ...
  fase_pagamentos AS (
    SELECT COALESCE(SUM(valor_bruto) FILTER (WHERE meio ILIKE '%conta assinada%'), 0)::numeric(14,2) as conta_assinada_valor
  ...
  fase_marketing_mensal AS (
    SELECT
      SUM(o_num_posts)::integer as o_num_posts, SUM(o_num_stories)::integer as o_num_stories,
      SUM(o_alcance)::integer as o_alcance, SUM(o_interacao)::integer as o_interacao,
      SUM(o_compartilhamento)::integer as o_compartilhamento, AVG(o_engajamento)::numeric(8,4) as o_engajamento,
      SUM(o_visu_stories)::integer as o_visu_stories, SUM(m_valor_investido)::numeric(14,2) as m_valor_investido,
      SUM(m_alcance)::integer as m_alcance, AVG(m_frequencia)::numeric(8,4) as m_frequencia,
      AVG(m_cpm)::numeric(10,2) as m_cpm, SUM(m_cliques)::integer as m_cliques,
      AVG(m_ctr)::numeric(8,4) as m_ctr, AVG(m_cpc)::numeric(10,2) as m_cpc,
  ...
  -- Atracao/Fat (mensal): espelha as linhas Artistico + Producao da Orcamentacao.
  -- Mesma fonte/filtros da fn_refresh_gold_orcamento (silver.lancamento_classificado,
  -- valor_bruto, por competencia, sem antecipacao Stone / ignorados) + consumacao de artistas.
  fase_atracao_orc AS (
    SELECT
      COALESCE((
        SELECT SUM(CASE WHEN lc.tipo_ca = 'DESPESA' THEN lc.valor_bruto ELSE -lc.valor_bruto END)
        FROM silver.lancamento_classificado lc
        WHERE lc.bar_id = p_bar_id
  ...
  SELECT
    ...
    (ca.atracoes_eventos + ca.freelas + ca.materiais + ca.manutencao + ca.utensilios + ca.alimentacao
      + ca.comissao + ca.cmo + ca.pro_labore + ca.ocupacao_custo + ca.imposto + ca.escritorio_central + ca.marketing_fixo + ca.adm_fixo),
    CASE WHEN p.faturamento_total > 0 THEN (ao.atracao_num / p.faturamento_total * 100)::numeric(6,2) END,
  ...
  LEFT JOIN fase_nps_digital nd ON true LEFT JOIN fase_nps_salao ns ON true
  LEFT JOIN fase_nps_reservas nr ON true CROSS JOIN fase_nps_criterios nc
  CROSS JOIN fase_tempos_agg t CROSS JOIN fase_mix mx
  CROSS JOIN fase_stockout stk CROSS JOIN fase_google g
  CROSS JOIN fase_custos ca CROSS JOIN fase_cancelamentos canc
  CROSS JOIN fase_pagamentos pg LEFT JOIN fase_marketing_mensal mk ON true
  CROSS JOIN fase_atracao_orc ao
