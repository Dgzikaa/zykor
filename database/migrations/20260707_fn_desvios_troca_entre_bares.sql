-- Item 2: o Desvio de Consumo passa a considerar as Trocas entre bares.
--   troca (líquida) = entrada (bar recebeu, financial.trocas.bar_destino)
--                   − saída  (bar enviou, financial.trocas.bar_origem)
-- Entra no balanço como uma compra/saída de estoque na data de competência da troca:
--   estoque_fim_teorico = ini + compras + TROCA + produzido − saída teórica − desperdício
-- Assim a mercadoria emprestada não vira perda falsa no emissor nem sobra falsa no recebedor.

DROP FUNCTION IF EXISTS gold.fn_desvios(integer, date, date);
...
AS $function$
  ...
  -- troca entre bares: entrada (recebeu) e saída (enviou) na data de competência
...
DROP FUNCTION IF EXISTS gold.fn_desvios_proteina(integer, date, date);
