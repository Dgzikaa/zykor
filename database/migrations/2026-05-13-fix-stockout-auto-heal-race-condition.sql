-- Fix race condition no auto-heal de stockout D-1 + bug discord_webhooks sem schema
-- ============================================================================
-- Contexto:
-- Discord recebeu alertas "🚨 Stockout D-1 sem dados (auto-heal falhou)" para
-- bars 3 e 4 em 2026-05-12 e 2026-05-13. Investigacao mostrou que NAO houve
-- falha real:
--   - net._http_response: ambas chamadas a stockout-processar retornaram 200
--     e success:true (650 e 209 produtos processados).
--   - silver bate com bronze (650/209) apos o auto-heal.
--
-- Causa raiz em revalidar_stockout_dia_anterior_ambos_bares_v2:
--   1) pg_sleep(10) curto demais — pg_net materializa _http_response.status_code
--      somente no fim da transacao do cron (todos os timestamps colapsam em um
--      mesmo instante). Durante o loop, status_code lido como NULL apesar da
--      edge function ter respondido 200.
--   2) Condicao de saida bugada:
--        EXIT WHEN v_count_silver_apos > 0 OR v_status_proc != 200 IS NULL;
--      Avalia (NULL != 200) IS NULL → true, entao SAI do loop assim que
--      status=NULL — sem retentar. E nao retenta no 500.
--   3) v_count_silver_apos contado antes do COMMIT do edge function ser visivel
--      por outra transacao. Resultado: alerta com status_code=null disparado
--      enquanto na verdade o silver ESTA sendo gravado.
--
-- Fix:
--   a) pg_sleep aumentado para 25s (mais cobertura).
--   b) Confianca apenas no silver count (status pg_net e' nao-confiavel dentro
--      da mesma transacao).
--   c) Recount silver com sleep extra antes de disparar alerta.
--
-- Bug secundario: enviar_alerta_discord_sistema(4 args) tem
--   FROM discord_webhooks (sem schema) com search_path='public' — quebra
--   ocasionalmente com "relation discord_webhooks does not exist". A versao
--   5-args (chamada por _dedup) ja' usa system.discord_webhooks corretamente.
--   Corrigir a 4-args defensivamente.
-- ============================================================================
    ...
    BEGIN
      ...
      ELSE
        ...
        IF v_count_silver = 0 THEN
          ...
          -- Tentar processar ate 2 vezes. Confiar APENAS no silver count
          -- (pg_net._http_response.status_code nao e' confiavel dentro da
          -- mesma transacao — colapsa em NULL ate o commit do cron).
          WHILE v_tentativas < 2 LOOP
            ...
            -- Verificar silver (criterio unico de sucesso)
          ...
          -- Se silver ainda zero, aguardar mais 15s e recontar antes de alertar
          -- (defesa contra COMMIT atrasado do edge function visivel so' depois)
...
-- ----------------------------------------------------------------------------
-- Fix defensivo: enviar_alerta_discord_sistema (4 args) usa
-- FROM discord_webhooks sem schema. Funciona somente quando search_path inclui
-- 'system'. Hoje nao tem chamadores diretos (apenas via _dedup que ja' usa a
-- 5-args), mas mantemos a funcao corrigida para nao quebrar futuro.
-- ----------------------------------------------------------------------------
