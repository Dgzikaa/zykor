-- fn_desvios: embalagem tem que vir da MESMA chave que a tela de Insumos usa.
--
-- Bug: o preco_emb lia insumo_unidade pela chave do VMarket
-- (iu.id_prod = b.id_produto_sisfood_cotacao), que guarda o valor AUTO derivado.
-- Mas a edição manual da equipe (tela de Insumos / silver.insumo_catalogo) grava na
-- chave iu.id_prod = -operations.insumos.id. Resultado: tudo que a equipe corrigiu na
-- mão (Red Bull 250ml, Monin 700ml, gins 750ml, Flying Fish 330ml, ~80 insumos no bar 4)
-- era IGNORADO pelo Desvio, que ficava preso no valor automático (1000/355/etc).
--
-- Fix: ler insumo_unidade em iu.id_prod = -i.id (idêntico a silver.insumo_catalogo),
-- mantendo o fallback derive_embalagem(nome, unidade) quando não houver edição manual.
-- Assim tela de Insumos e Desvio sempre batem.
