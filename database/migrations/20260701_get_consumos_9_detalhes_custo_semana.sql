-- Controle de Consumação: detalhe linha-a-linha das 9 categorias + CUSTO REAL por linha.
-- Se o produto tem ficha técnica → custo = custo_ficha × qtd × (desconto / (desconto + valorfinal));
-- senão → desconto × fator (0,35). Une a classificação de get_consumos_9_detalhes_semana com
-- o custo de get_consumo_custo_real_semana. Também devolve tem_ficha.
-- Paginado via p_limit/p_offset (cada página ≤1000 → imune ao cap de 1000 do PostgREST) com
-- ordenação determinística pra o offset ser estável. p_limit null = tudo.
drop function if exists public.get_consumos_9_detalhes_custo_semana(integer, date, date, text, numeric);
drop function if exists public.get_consumos_9_detalhes_custo_semana(integer, date, date, text, numeric, integer, integer);

create function public.get_consumos_9_detalhes_custo_semana(
