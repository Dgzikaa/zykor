-- Função: admin_get_api_credentials
-- Descrição: Busca credenciais de API por bar_id (com ou sem filtro de sistema/ambiente)
-- Categoria: SUPORTE/ADMIN

-- Versão 1: Apenas por bar_id
CREATE OR REPLACE FUNCTION public.admin_get_api_credentials(p_bar_id integer)
...
BEGIN
    RETURN QUERY
    SELECT 
        ac.id, ac.bar_id, ac.sistema, ac.ambiente, ac.client_id, ac.client_secret,
        ac.redirect_uri, ac.scopes, ac.base_url, ac.access_token, ac.refresh_token,
        ac.token_type, ac.expires_at, ac.authorization_code, ac.oauth_state,
        ac.empresa_id, ac.empresa_nome, ac.empresa_cnpj, ac.last_token_refresh,
        ac.token_refresh_count, ac.ativo, ac.criado_em, ac.atualizado_em
    FROM public.api_credentials ac
    WHERE ac.bar_id = p_bar_id;
END;
$function$;
