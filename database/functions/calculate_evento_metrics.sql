...
--
-- Sincronizado com a funcao REAL deployada em producao (versao_calculo = 28) em 2026-06-04.
-- O arquivo anterior estava muito desatualizado (lia 'nibo_agendamentos', tabela que nem
-- existe mais). Fonte da verdade = pg_get_functiondef no banco; este arquivo apenas reflete.
--
...
-- Ver migration 20260604_c_art_prod_coalesce_valor_pago.sql.
...
BEGIN
  ...
  -- Regras historicas Deboche (bar 4):
  --  cozinha: t0_t2 ate 19/03, t0_t3 de 20/03 em diante
  --  bar:     t0_t2 ate 06/03, t0_t3 de 07/03 em diante
  -- Bar 3 (Ordinario): cozinha sempre t0_t2, bar sempre t0_t3
  ...
  -- Custo Atracao/Producao do Conta Azul.
  -- COALESCE(NULLIF(valor_pago,0), valor_bruto): pago quando ja pago (bate CA na virgula),
  -- bruto quando a pagar (valor_pago=0) p/ nao zerar a atracao da semana corrente.
  ...
  IF NOT is_evento_especial THEN
    ...
    WITH tempo_cozinha AS (
      ...
      SELECT CASE WHEN v_use_t0t3_bar THEN t.t0_t3 ELSE t.t0_t2 END AS tempo
      FROM bronze.bronze_contahub_produtos_temposproducao t
      INNER JOIN operations.produto_categoria_mix pcm ON pcm.bar_id = t.bar_id AND pcm.loc_desc = t.loc_desc
      WHERE t.data = evento_record.data_evento AND t.bar_id = evento_record.bar_id AND pcm.categoria IN ('DRINK','BEBIDA')
    )
    SELECT
  ...
  UPDATE operations.eventos_base SET
    ...
    calculado_em = NOW(), precisa_recalculo = FALSE, atualizado_em = NOW(), versao_calculo = 28
  WHERE id = evento_id;
END;
$function$;
