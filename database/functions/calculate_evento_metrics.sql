...
--
-- Sincronizado com a funcao REAL deployada em producao (versao_calculo = 34) em 2026-07-23.
-- Fonte da verdade = pg_get_functiondef no banco; este arquivo apenas reflete.
--
-- v34 (2026-07-23): "Clientes" (cl_real) passou de "quem PAGOU" para "quem CONSUMIU".
--   Apos a virada operacional pro cartao individual (~08/07/2026), a galera junta a conta num
--   cartao so; os demais cartoes ficam com vd_vrpagamentos=0 mas com vd_pessoas>0 (consumo e
--   couvert migraram pro cartao pagador) = clientes reais. A regra antiga (vd_vrpagamentos>0)
--   derrubava esses ~60/dia e inflava o ticket medio. Nova regra conta a pessoa se PAGOU, ou se
--   o cartao ficou 100% vazio (0 item, 0 desconto) com pessoa (= conta juntada); CORTESIA
--   (pgto 0 COM item/desconto) continua FORA. Mesma regra em pessoas_ate_19h/20h.
--   Recalculado todo o 2026 (bares 3 e 4). Ver memoria project_planejamento_clientes_pagantes_definicao.
--
-- c_art / c_prod usam COALESCE(NULLIF(valor_pago,0), valor_bruto) da bronze_contaazul_lancamentos
-- (categorias 'Atrações Programação'/'Atrações/Eventos' = artistico; 'Produção Eventos' = producao).
...
BEGIN
  ...
  -- Clientes = quem CONSUMIU, nao apenas quem PAGOU no proprio cartao (v34).
  -- Apos a virada pro cartao individual (~08/07/2026), juntam a conta num cartao so; os demais
  -- cartoes ficam com vd_vrpagamentos=0 mas com vd_pessoas>0 (consumo/couvert migraram pro pagador).
  -- Regra: conta se PAGOU, ou se o cartao ficou 100% vazio (0 item, 0 desconto) com pessoa
  -- (= conta juntada). CORTESIA (pgto 0 COM item/desconto = consumo comp) continua FORA.
  SELECT COALESCE(SUM(CASE WHEN vd_vrpagamentos > 0
                             OR (COALESCE(vd_qtditens,0) = 0 AND COALESCE(vd_vrdescontos,0) = 0 AND vd_pessoas > 0)
                           THEN vd_pessoas ELSE 0 END), 0)::INTEGER AS total_pessoas_pagantes,
         COALESCE(SUM(vd_vrcouvert), 0)::NUMERIC AS total_couvert,
         COALESCE(SUM(CASE WHEN (vd_vrpagamentos > 0
                                  OR (COALESCE(vd_qtditens,0) = 0 AND COALESCE(vd_vrdescontos,0) = 0 AND vd_pessoas > 0))
                                AND EXTRACT(HOUR FROM vd_hrabertura) BETWEEN 5 AND 18 THEN vd_pessoas ELSE 0 END), 0)::INTEGER AS pessoas_ate_19h,
         COALESCE(SUM(CASE WHEN (vd_vrpagamentos > 0
                                  OR (COALESCE(vd_qtditens,0) = 0 AND COALESCE(vd_vrdescontos,0) = 0 AND vd_pessoas > 0))
                                AND EXTRACT(HOUR FROM vd_hrabertura) BETWEEN 5 AND 19 THEN vd_pessoas ELSE 0 END), 0)::INTEGER AS pessoas_ate_20h
    INTO contahub_per
  ...
  IF NOT is_evento_especial THEN
    ...
    SELECT
      ...
      (SELECT COUNT(*)::INTEGER FROM tempo_cozinha WHERE tempo > 1200 AND tempo <= 3600) AS atrasao_cozinha,
      (SELECT COUNT(*)::INTEGER FROM tempo_bar      WHERE tempo > 600 AND tempo <= 3600 ) AS atrasao_bar
  ...
  UPDATE operations.eventos_base SET
    ...
    calculado_em = NOW(), precisa_recalculo = FALSE, atualizado_em = NOW(), versao_calculo = 34
  WHERE id = evento_id;
END;
$function$;
